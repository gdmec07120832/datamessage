    ...
    <div style="background-color: #FFFFFF;margin-bottom: 10px;border-radius : 5px;height:180px;">
      <van-row type="flex"  style="height:90px;padding-top: 10px;" justify="center">
        <van-col span="7" class="span_div">
          <div class="col_div">
            <div class="num_text" style="color:#396FFF;">{{(form.sendTargetStr*1||0).toFixed(2)}}亿</div>
      ...
      <van-row type="flex" justify="center" style="height:90px;">
        <van-col span="7" class="span_div">
          <div class="col_div">
            <div class="num_text" style="color:#396FFF;">{{(form.dailySurplusTargetStr==null||form.dailySurplusTargetStr==0)?'--':(form.dailySurplusTargetStr+'万')}}</div>
            <!-- <div class="num_text" style="color:#396FFF;">{{form.dailySurplusTargetStr}}</div> -->
...
export default {
  ...
  methods: {
    getData(dateString) {
      ...
      this.$axios.get('/bi-mobile/api/admin/biDeliveryReport/getBaseData2', {
      ...
      this.$axios.get('/bi-mobile/api/admin/biDeliveryReport/getBiPendingTrial2', {
        params: { dateStr: dateString },
      }).then(res => {
        let labelRet = [];
        let dataRet = [];
      ...
      this.$axios.get('/bi-mobile/api/admin/biDeliveryReport/getMatchingRate2', {
        params: { dateStr: dateString },
      }).then(res => {
        let labelRet = [];
        let jjdataRet = [];
